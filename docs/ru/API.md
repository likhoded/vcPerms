...
Запросы идут в глобальном контексте из `config.yml`, плюс необязательные `world` / `dimension` в запросе. Если их нет — берём последний мир игрока (вход / смена мира).
