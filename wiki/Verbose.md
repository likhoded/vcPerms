...
`on` logs every permission check into a ring buffer (2000 entries). `record` clears the buffer and optionally filters by player name or node substring. `paste` / `upload` writes `exports/verbose-<unix>.json` and turns verbose off.
